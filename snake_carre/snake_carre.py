    ...
    if situation == "vivant" and jeux_situation%2:
        ...
        for i in can.find_withtag("bonus"):
            if collision(i,tete)<0:
                ...
                fen.title(f"{score}")
                create_bonus_carre()
                liste_corps.append(can.create_rectangle(list_coords[-1],fill="red",tag="corps"))
    can.after(time_after,move_snake_carre)
...
for i in range(taille_debut):
    if i == 0:
        tete = can.create_rectangle(p-rayon-(i*rayon*2),p-rayon,p+rayon-(i*rayon*2),p+rayon,fill="blue",tag="tete")
        liste_corps.append(tete)
    else:
        corps = can.create_rectangle(p-rayon-(i*rayon*2),p-rayon,p+rayon-(i*rayon*2),p+rayon,fill="red",tag="corps")
